...
Current checkpoint: judge-requested frontend recovery remediation. Contract deployment remains unchanged; repaired frontend revision `4bd114d92428b7cc8f0e7f88e95b99e9389c5b18` passed local verification and exact-release Vercel recovery E2E.
...
| Recover frontend transactions that outlive the UI timeout | The prior UI retained a hash but left the operation in an internal `READBACK` state with no public reconciliation control; a finalized Draft write could remain visibly unresolved and another attempt risked confusion | Revision `4bd114d92428b7cc8f0e7f88e95b99e9389c5b18` persists the hash, restores it on reload, exposes `RECONCILIATION_REQUIRED` plus `Continue verification`, queries GenLayer transaction state, and refuses readback or success until finality and recognized semantic success. Recoverable results are typed centrally so no write component renders them as a failure. The classifier accepts `FINALIZED` + `MAJORITY_AGREE` + successful leader execution while correctly ignoring validators cancelled after quorum. Successful Create recovery opens the authoritative Draft and its Freeze control; successful Freeze recovery remounts the open detail from authoritative state. Exact-release Create `0xd087ee…fb01a2e` and Freeze `0x4d70ea…00e28b5` proved same-hash recovery, no duplicate writes, no false red banners, immediate FROZEN refresh, disconnected reload, and authoritative readback. Regression coverage includes timeout, unresolved, semantic ambiguity, quorum cancellation, finalized success/failure, refresh, retry, casing, and duplicates. |
...
- Final implementation commit under review: `4bd114d92428b7cc8f0e7f88e95b99e9389c5b18`
- Evidence reconciliation commit: `b549897876a2f3891efcd5cb0bb025f1fa323d80`
- Final frontend application revision: `4bd114d92428b7cc8f0e7f88e95b99e9389c5b18`; deployed contract source remains revision `218f969234afef728551dba1b6d086a579304188` with the locked SHA-256 above.
...
Measured exact-release evidence: production deployment `dpl_EP7YeHtpmxU36xee33KdTCrqhbvr` retained Create hash `0xd087ee514811a72afac612e919dc89a1f0da466ac1d7bef2ecdafb6e1fb01a2e` while semantic data was incomplete. The UI showed only the pending recovery bar, no component error banner, and no duplicate write. Once independently verified `FINALIZED`, `MAJORITY_AGREE`, and leader `SUCCESS`, `Continue verification` reused the hash, completed bounded authoritative readback, and opened `trg-0012` in `DRAFT` with the Freeze control. Freeze hash `0x4d70ea57231c6c4d207bb45fda4b6e071dd2b4b12790b534d823d460000e28b5` followed the same recoverable path. After finality and readback, the open detail immediately changed to `FROZEN` and removed the Freeze action. A clean reload started disconnected with no pending stage and showed exactly one `trg-0012` among twelve registry entries.
